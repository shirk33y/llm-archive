...
The language server uses a dynamic subdomain prefix (e.g., `x.localhost`, `t.localhost`) that changes on each restart:
...
- **Changes on restart**: Each Windsurf restart generates a new single-letter prefix
- **Extracted via CDP**: The current hostname can be extracted from intercepted requests via CDP

The subdomain generation logic is not present in the beautified extension JavaScript code - it must be in the compiled language server binary or in the connect-transport library.
